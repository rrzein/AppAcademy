class Mastermind
    ...
    until game_over? || win?
      ...
      until valid_move
        begin
          ...
          puts "#{move} was not a valid guess."
          puts "Error was: #{error.message}"
        end
      end
  ...
  def get_move
    ...
    unless is_valid_move?(input)
      raise IOError.new("Please enter a string of 4 letters out of R, G, B, Y, O, P.")
    end
    input
  end

  def is_valid_move?(input)
    input.length == 4 && input.split(//).uniq - COLORS.keys.map(&:to_s) == []
  end
